...
function build_windows_2012 {
  if [ "$BUILDER" == "skip-build" ]; then
    packer validate -var-file=vars.json windows_2012.json
  else
    packer validate $BUILDER -var-file=vars.json windows_2012.json
  fi
  ...
  if [ -z "$CONTINUOUS_INTEGRATION" ]; then
    humanize-repo-url
    if [ ! -z "$GIT_REPO_URL" ]; then REPO_URL_VAR="--var packer_windows_2012_repo=$GIT_REPO_URL"; fi
    ...
    if [ "$BUILDER" != "skip-build" ]; then
      PACKER_LOG=$PACKER_LOG_VALUE packer build $BUILDER $REPO_URL_VAR -var-file=vars.json windows_2012.json
      ...
      if [[ "$BUILDER" == *docker* ]]; then
        ...
        extract_from_json "WINDOWS_2012_VERSION" "packer_windows_2012_version" "windows_2012"

        # Write out the simple fig.yml file
        if [ ! -z "$DOCKER_USER" ]; then
          echo "windows_2012:" > fig.yml
          echo "  image: $DOCKER_USER/packer-windows_2012:$WINDOWS_2012_VERSION" >> fig.yml
    ...
    WINDOWS_2012_ADMIN_PASSWORD=`openssl rand -base64 12`
    WINDOWS_2012_SECRET_KEY=`openssl rand -base64 12`

    PACKER_LOG=$PACKER_LOG_VALUE packer -machine-readable build $BUILDER \
      -var "windows_2012_admin_password=${WINDOWS_2012_ADMIN_PASSWORD}" \
      -var "windows_2012_secret_key_password=${WINDOWS_2012_SECRET_KEY}" \
      windows_2012.json | tee packer.log
  fi
}
...
if hash strip-json-comments 2>/dev/null; then
  strip-json-comments packer-windows_2012.json > windows_2012.json
  build_windows_2012
elif [ -f windows_2012.json ]; then
  build_windows_2012
else
  echo "  strip-json-comments needs to be installed to generate the windows_2012.json file"
